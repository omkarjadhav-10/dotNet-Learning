...
# Repo-Linked Examples (GameStore API)

## REST API Fundamentals (.NET Web API) -> examples from this repo

- Resource + route design: `Endpoints/GamesEndpoints.cs` uses `app.MapGroup("/games")` and maps `GET`, `POST`, `PUT`, `DELETE` on the same resource.
- Correct status codes:
  - `GET /games/{id}` returns `404` when not found (`Results.NotFound()`).
  - `POST /games` returns `201 Created` with `CreatedAtRoute("GetGame", ...)`.
  - `PUT` and `DELETE` return `204 NoContent` on success.
- Path parameters: `/{id}` is used for single-resource operations.
- JSON contracts: request/response payloads are represented with DTO records in `Dtos/`.

## Data Management & Architecture in ASP.NET Core -> examples from this repo

- Entities (DB models): `Entities/Game.cs`, `Entities/Genre.cs`.
- DTO boundaries:
  - Input: `Dtos/CreateGameDto.cs`, `Dtos/UpdateGameDto.cs`.
  - Output: `Dtos/GameSummaryDto.cs`, `Dtos/GameDetailsDto.cs`.
- Validation:
  - Data annotations on DTOs (`Required`, `StringLength`, `Range`).
  - Custom attribute `Dtos/NotDefaultDateOnlyAttribute.cs` prevents default `DateOnly`.
  - Endpoint-level enforcement via `.WithParameterValidation()`.
- Separation of concerns:
  - API handlers in `Endpoints/GamesEndpoints.cs`.
  - Persistence in `Data/GameStoreContext.cs`.
  - Mapping in `Mapping/GameMapping.cs`.

## Entity Framework Core (EF Core) Guide -> examples from this repo

- DbContext registration: `Program.cs` configures `GameStoreContext` with SQLite using connection string `GameStore`.
- DbSets: `Games` and `Genres` in `Data/GameStoreContext.cs`.
- Relationship use: queries include genre data with `.Include(game => game.Genre)`.
- Projection pattern: list endpoint uses `.Select(game => game.ToGameSummaryDto())` before `ToListAsync()`.
- Migrations present in repo:
  - `Migrations/20260211071554_InitialCreate.cs`
  - `Migrations/20260211110000_SeedGenres.cs`
- Seed data: `OnModelCreating` seeds default genres.

## Advanced Architecture Concepts in ASP.NET Core -> examples from this repo

- Dependency Injection in practice:
  - `GameStoreContext` is injected directly into route handlers.
  - `builder.Services.AddDbContext<GameStoreContext>(...)` wires scoped DbContext lifecycle.
- Configuration:
  - Connection string resolved from `appsettings.json` through `builder.Configuration.GetConnectionString("GameStore")`.
- Async I/O patterns:
  - Endpoints consistently use `await` with EF methods like `FindAsync`, `FirstOrDefaultAsync`, `SaveChangesAsync`.

## Code Organization & Integration in ASP.NET Core -> examples from this repo

- Route group organization: all game routes are centralized in `Endpoints/GamesEndpoints.cs`.
- Extension-based composition:
  - `app.MapGamesEndpoints()` keeps `Program.cs` minimal.
  - Mapping extensions in `Mapping/GameMapping.cs` keep handlers clean.
- API testing workflow:
  - Ready-made HTTP requests in `requests/game.http` for CRUD verification.

## Repo quick walk-through by request flow

1. Request hits `/games` route in `Endpoints/GamesEndpoints.cs`.
2. DTO validation runs for body-bound models (e.g., `CreateGameDto`).
3. Handler queries or updates via `GameStoreContext`.
4. Entities are converted with mapping extensions in `Mapping/GameMapping.cs`.
5. Response returns DTO + proper HTTP status code.
